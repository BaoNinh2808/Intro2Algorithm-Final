...
bool isInList(vector<int>& vertex, int val){
    for (int i = 0; i < vertex.size(); i++){
        if (vertex[i] == val) return true;
    }
    return false;
}

vector<vector<int>> tao_thanh_phan_lien_thong_chi_chua_cac_dinh_va_khong_chua_canh(vector<vector<int>>& graph, int N, vector<int>& vertex, vector<int> eliminate_road){
    vector<bool> isUsedVetex(N+1, false);    //tạo mảng đánh dấu các phần tử đã được dùng
    vector<vector<int>> components;
    for (vector<int> road : graph){
        int u = road[0];
        int v = road[1];
        if ((u == eliminate_road[0] && v == eliminate_road[1]) || (u == eliminate_road[1] && v == eliminate_road[0])){
            //bỏ cạnh u-v trong graph
            continue;
        }
        if (isInList(vertex, u)){
            int u_componentIndex = findComponentIndex(components, u);  //index của component chứa u - trả về component.size() nếu chưa có
            int v_componentIndex = findComponentIndex(components, v);  //index của component chứa v - trả về component.size() nếu chưa có
    ...
    for (int v : vertex){
        if (isUsedVetex[v] == false){
            vector<int> new_component;
            new_component.push_back(v);
            components.push_back(new_component);
        }
    }
    return components;
...
vector<vector<int>> dem_so_thanh_pho_khong_den_duoc_voi_nhau_khi_bo_moi_con_duong(vector<vector<int>>& graph, int N, int M){
    ...
    vector<int> vertexList = createVector(N);
    vector<vector<int>> components = tao_thanh_phan_lien_thong_chi_chua_cac_dinh_va_khong_chua_canh(graph, N, vertexList, vector<int> () = {-1,-1});
    for (int i = 0; i < M; i++){
        vector<int> road = graph[i];
        int u = road[0];
        int u_componentIndex = findComponentIndex(components, u);  //index của component chứa u - trả về component.size() nếu chưa có
        vector<int> oldComponent = components[u_componentIndex];
        vector<vector<int>> newComponents = tao_thanh_phan_lien_thong_chi_chua_cac_dinh_va_khong_chua_canh(graph, N, oldComponent, road);
        components.erase(components.begin() + u_componentIndex);
        ...
        components.erase(components.end() - newComponents.size(), components.end());
        components.push_back(oldComponent);
    }
    return res;
}
